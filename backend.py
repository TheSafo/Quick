...
class Profiles(Resource):
    ...
    def post(self):
    	data = request.get_json(force=True)
        cur.execute("INSERT INTO quickschemas.profiles (id, name, token) VALUES (%s, %s, %s) ON CONFLICT DO UPDATE SET name=%s, token=%s",
        		   (data['id'], data['name'], data['token'], data['name'], data['token']))
        conn.commit()
        return 'success'
...
class Request(Resource):
	def post(self):
		...
		# cur.execute("""SELECT l. FROM quickschemas.locations l WHERE ST_DWithin(l.pt, ST_SetSRID(ST_Point(%s, %s),4326)::geography, 250)
		# 														AND l.time BETWEEN %s - (20 * interval '1 minute') AND %s""",
		# 	(fromlon, fromlat, t, t))
		# for 
		#to_char(l.time, 'D')=to_char(%s, 'D')
